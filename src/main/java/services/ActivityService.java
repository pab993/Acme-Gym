...
public class ActivityService {
	...
	public Activity reconstruct(Activity activity, BindingResult binding) {
		...
		comprobarImagenes(activity.getPictures(), binding);
	...
	public boolean comprobarImagenes(Collection<String> pictures, BindingResult binding) {
		FieldError error;
		String[] codigos;
		...
		for (String p : pictures) {
			...
			if (!p.matches("^(?:http(s)?:\\/\\/)?[a-zA-Z0-9_.-]+(?:[.][a-zA-Z0-9_\\.-]+)+[a-zA-Z0-9-._~:\\/?#[/]@!/$&'/(/)/*/+,;=.]+?$")) {
				result = true;
				break;
			}
		}
		...
		if (result) {
			codigos = new String[1];
			codigos[0] = "activity.pictures.error";
			error = new FieldError("activity", "pictures", pictures, false, codigos, null, "");
			binding.addError(error);
